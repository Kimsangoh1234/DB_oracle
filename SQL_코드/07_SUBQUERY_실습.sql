/***** SUBQUERY 실습 *****/
--1.각 직원의 급여가 부서 평균 급여보다 높은 직우너들의 이름과 급여 조회하기
--1) 부서 평균급여 구하기
SELECT AVG(SALARY)FROM EMPLOYEE2 E WHERE DEPT_CODE = E.DEPT_CODE;
--2) 직원들의 이름과 급여 조회
...
/*
서브쿼리
SELECT MIN(SALARY) --> 컬럼 선택
FROM EMPLOYEE2     --> 찾을 데이터 테이블 선택
WHERE SALARY >=3000000; --> 급여가 300만원 이상인 행들만 최소값을 계산

외부쿼리 = 메인쿼리
SELECT EMP_NAME,SALARY  --> 최종으로 눈으로 볼 컬럼 선택
FROM EMPLOYEE2          --> 테이블 선택
WHERE SALARY            --> 급여가 서브쿼리에서 반환된 최소 급여와 같은 행들만 가져오는 것
*/
/*SELECT를 활용한 서브쿼리 실습*/
