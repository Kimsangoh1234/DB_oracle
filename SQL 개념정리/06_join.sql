...
-- ANSI표준
SELECT EMP_NAME,DEPT_TITLE
FROM EMPLOYEE2 LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
...
-- 왼쪽에 있는 컬럼에 수에 맞게 표시를 넣어라(NULL값이라도 넣어라)
...
-- ANSI표준
SELECT EMP_NAME,DEPT_TITLE
FROM EMPLOYEE2 RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
...
-- ANSI표준
SELECT EMP_NAME,DEPT_TITLE
FROM EMPLOYEE2 FULL JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
